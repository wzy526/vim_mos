...
class MOSInnerFnNoOutProj(torch.autograd.Function):

    @staticmethod
    @custom_fwd
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight, num_experts,
                A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
                C_proj_bias=None, delta_softplus=True, checkpoint_lvl=1):
        """
             xz: (batch, dim, seqlen)
        ...
        if not isinstance(A, torch.Tensor):
            raise TypeError(f"A must be a tensor, got {type(A)}")
            
        assert checkpoint_lvl in [0, 1]
        L = xz.shape[-1]
        delta_rank = delta_proj_weight.shape[1]
        d_state = A.shape[-1] * (1 if not A.is_complex() else 2) * num_experts
        if torch.is_autocast_enabled():
            x_proj_weight = x_proj_weight.to(dtype=torch.get_autocast_gpu_dtype())
            delta_proj_weight = delta_proj_weight.to(dtype=torch.get_autocast_gpu_dtype())
        if xz.stride(-1) != 1:
        ...
        if B is None:  # variable B
            ...
            if not A.is_complex():
                ...
                B = rearrange(B, "(b l) (dstate two) -> b 1 dstate (l two)", l=L, two=2).contiguous() # (b, 1, dstate, l)
        else:
            if B.stride(-1) != 1:
                B = B.contiguous()
        if C is None:  # variable C
            C = x_dbl[:, -d_state:]  # (bl dstate)s
            if C_proj_bias is not None:
                C = C + C_proj_bias.to(dtype=C.dtype)
            if not A.is_complex():
                # C = rearrange(C, "(b l) dstate -> b dstate l", l=L).contiguous()
                C = rearrange(C, "(b l) dstate -> b 1 dstate l", l=L).contiguous() # (b, 1, dstate, l)
            else:
                C = rearrange(C, "(b l) (dstate two) -> b 1 dstate (l two)", l=L, two=2).contiguous()
        else:
            if C.stride(-1) != 1:
        ...
        # wzy的魔改开始 
        # spliT A, B, C, D into sub-ssms
        A_i = []
        for i in range(num_experts):
            A_i.append(A[:, i, :]) # A_i shape: (d_inner, d_expert)
        d_expert = A_i[0].shape[-1]

        # 将B的d_state维度分解成num_heads个d_head维度
        # B shape: (b, 1, dstate, l) -> (b, 1, num_experts, d_expert, l)
        B = B.view(B.shape[0], B.shape[1], num_experts, d_expert, B.shape[-1])
        # B = B.view(B.shape[0], num_experts, d_expert, B.shape[-1])  # (b, num_heads, d_head, seqlen)
        B_i = [] # num_heads个B_i
        for i in range(num_experts):
            B_i.append(B[:, :, i, :, :])  # 每个B_i的形状为 (b, 1, d_expert, seqlen), B_i = num_experts个(b, 1, d_expert, seqlen)
        
        # C = C.view(C.shape[0], num_experts, d_expert, C.shape[-1])  # (b, num_experts, d_expert, seqlen)
        C = C.view(C.shape[0], C.shape[1], num_experts, d_expert, C.shape[-1]) # (b, 1, num_experts, d_expert, seqlen)
        # num_experts个C_i
        C_i = []
        for i in range(num_experts):
            C_i.append(C[:, :, i, :, :])  # C_i shape (b, 1, d_expert, seqlen)
        ...
        for i in range(num_experts):
            ...
            out, scan_intermediates, out_z = selective_scan_cuda.fwd(
                conv1d_out, delta, A_i[i], B_i[i], C_i[i], D, z, delta_bias, delta_softplus
            )
            out_list.append(out_z[torch.newaxis, :, :, :])
        out_list = torch.cat(out_list, dim=0)

        # 保存conv1d的结果用于返回
        conv1d_result = conv1d_out.clone().detach() if conv1d_out is not None else None
        ...
        ctx.save_for_backward(xz, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight,
                              ...
                              A, B, C, D, delta_bias, scan_intermediates, out)
        # return rearrange(out_z, "b d l -> b l d")
        return conv1d_result, out_list

    @staticmethod
    @custom_bwd
    def backward(ctx, dconv1d_result, dout):
        print("conv1d_result", dconv1d_result.shape)
        ...
        A = A.view(A.shape[0], -1)  # (dim, num_expert, d_expert) -> (dim, num_expert*d_expert)
        B = B.view(B.shape[0], B.shape[1], -1, B.shape[-1]) # (bs, ngroup, num_experts, d_expert, seqlen) -> (bs, ngroup, num_experts*d_expert, seqlen)
        C = C.view(C.shape[0], C.shape[1], -1, C.shape[-1]) # (bs, ngroup, num_expert, d_expert, seqlen) -> (bs, ngroup, num_expert*d_expert, seqlen)
        ...
        dconv1d_bias = dconv1d_bias if conv1d_bias is not None else None
        dconv1d_weight = rearrange(dconv1d_weight, "d w -> d 1 w")
        return (dxz, dconv1d_weight, dconv1d_bias, dx_proj_weight, ddelta_proj_weight, None,
                dA, dB, dC, dD,
                ddelta_bias if delta_bias is not None else None,
                dB_proj_bias, dC_proj_bias, None)
...
def mamba_inner_fn_no_out_proj(
    ...
    return MambaInnerFnNoOutProj.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus)
def mos_inner_fn_no_out_proj(
    xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight, num_experts,
    A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True
):
    return MOSInnerFnNoOutProj.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight, num_experts,
                                    A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus)
